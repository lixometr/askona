...
// once: true
  ...
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection8Js(module,exports){var Toggle=function(){function Toggle(){_classCallCheck(this,Toggle);this.items=$('.s8-map__links li a');this.mapEl=$('.s8-map__main-inner');this.listEl=$('.s8-map__list');this.parent=$('.s8-map');this.init();}_createClass(Toggle,[{key:'openMap',value:function openMap(){// this.mapEl.show()
// this.listEl.hide()
this.parent.removeClass('s8-map__main_list');}},{key:'openList',value:function openList(){// this.mapEl.hide()
// this.listEl.show()
this.parent.addClass('s8-map__main_list');}},{key:'addListeners',value:function addListeners(){var self=this;this.items.on('click',function(e){e.preventDefault();if($(this).parent().index()===0){self.openMap();}else{self.openList();}});}},{key:'init',value:function init(){this.addListeners();}}]);return Toggle;}();new Toggle();/***/},/***/"./src/js/cursorFollow.js":/*!********************************!*\
  !*** ./src/js/cursorFollow.js ***!
  \********************************//*! no static exports found *//***/function srcJsCursorFollowJs(module,exports){var CursorFollow=function(){function CursorFollow(){_classCallCheck(this,CursorFollow);this.el=document.querySelector('#cursor-follow');this.width=this.el.offsetWidth;this.height=this.el.offsetHeight;this.targetElements=['button','a','.hover','input','select'];this.init();}_createClass(CursorFollow,[{key:'makeActive',value:function makeActive(){this.el.classList.add('active');console.log('active');}},{key:'removeActive',value:function removeActive(){this.el.classList.remove('active');}},{key:'addListeners',value:function addListeners(){var _this4=this;document.body.addEventListener('mousemove',function(e){var x=e.clientX;var y=e.clientY;_this4.el.style.left=x-_this4.width/2+'px';_this4.el.style.top=y-_this4.height/2+'px';});this.targetElements.forEach(function(selector){var elements=[].concat(_toConsumableArray(document.querySelectorAll(selector)));elements.forEach(function(el){el.addEventListener('mouseover',function(){_this4.makeActive();});el.addEventListener('mouseout',function(){_this4.removeActive();});});});}},{key:'init',value:function init(){this.addListeners();}}]);return CursorFollow;}();new CursorFollow();/***/},/***/"./src/js/menu.js":/*!************************!*\
  !*** ./src/js/menu.js ***!
...
ctx.fill();}},{key:'render',value:function render(){var _this16=this;requestAnimationFrame(function(){//clears canvas
_this16.clearCanvas();//redraws stars
for(var i=0;i<_this16.numStars;i++){if(_this16.direction==='horizontal'){_this16.stars[i].xPos-=_this16.stars[i].getVelocity().x;}else{_this16.stars[i].yPos-=_this16.stars[i].getVelocity().y;}//if star goes off screen from the left call reset method
if(_this16.stars[i].xPos<0||_this16.stars[i].xPos>_this16.canvas.width){_this16.resetStar(i);}else if(_this16.stars[i].yPos<0||_this16.stars[i].yPos>_this16.canvas.height){_this16.resetStar(i);}else{_this16.draw(i);}}_this16.render();});}// reset the star xPos with a randon YPos 
