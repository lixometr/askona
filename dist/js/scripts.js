...
/******/return __webpack_require__(__webpack_require__.s="./src/js/scripts.js");/******/})(/************************************************************************//******/{/***/"./node_modules/gsap/CSSPlugin.js":/*!****************************************!*\
...
endUnit=Object(_gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["getUnit"])(endValue)||(p in _gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["_config"].units?_gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["_config"].units[p]:startUnit);startUnit!==endUnit&&(startNum=_convertToUnit(target,p,startValue,endUnit));this._pt=new _gsap_core_js__WEBPACK_IMPORTED_MODULE_0__["PropTween"](this._pt,isTransformRelated?cache:style,p,startNum,relative?relative*endNum:endNum-startNum,endUnit==="px"&&vars.autoRound!==false&&!isTransformRelated?_renderRoundedCSSProp:_renderCSSProp);this._pt.u=endUnit||0;if(startUnit!==endUnit){//when the tween goes all the way back to the beginning, we need to revert it to the OLD/ORIGINAL value (with those units). We record that as a "b" (beginning) property and point to a render method that handles that. (performance optimization)
...
TweenMaxWithCSS=gsapWithCSS.core.Tween;/***/},/***/"./node_modules/jquery/dist/jquery.js":/*!********************************************!*\
...
if(typeof noGlobal==="undefined"){window.jQuery=window.$=jQuery;}return jQuery;});/***/},/***/"./node_modules/lodash.throttle/index.js":/*!***********************************************!*\
  !*** ./node_modules/lodash.throttle/index.js ***!
  \***********************************************//*! no static exports found *//***/function node_modulesLodashThrottleIndexJs(module,exports,__webpack_require__){/* WEBPACK VAR INJECTION */(function(global){/**
 ...
 */function toNumber(value){if(typeof value=='number'){return value;}if(isSymbol(value)){return NAN;}if(isObject(value)){var other=typeof value.valueOf=='function'?value.valueOf():value;value=isObject(other)?other+'':other;}if(typeof value!='string'){return value===0?value:+value;}value=value.replace(reTrim,'');var isBinary=reIsBinary.test(value);return isBinary||reIsOctal.test(value)?freeParseInt(value.slice(2),isBinary?2:8):reIsBadHex.test(value)?NAN:+value;}module.exports=throttle;/* WEBPACK VAR INJECTION */}).call(this,__webpack_require__(/*! ./../webpack/buildin/global.js */"./node_modules/webpack/buildin/global.js"));/***/},/***/"./node_modules/selectric/public/jquery.selectric.js":/*!***********************************************************!*\
  ...
  \***********************************************************//*! no static exports found *//***/function node_modulesSelectricPublicJquerySelectricJs(module,exports,__webpack_require__){var __WEBPACK_AMD_DEFINE_FACTORY__,__WEBPACK_AMD_DEFINE_ARRAY__,__WEBPACK_AMD_DEFINE_RESULT__;/*!
 *         ,/
 *       ,'/
 *     ,' /
 *   ,'  /_____,
...
/[\xFD-\xFF]/g// y
   ...
   */var Selectric=function Selectric(element,opts){var _this=this;_this.element=element;_this.$element=$(element);_this.state={multiple:!!_this.$element.attr('multiple'),enabled:false,opened:false,currValue:-1,selectedIdx:-1,highlightedIdx:-1};_this.eventTriggers={open:_this.open,close:_this.close,destroy:_this.destroy,refresh:_this.refresh,init:_this.init};_this.init(opts);};Selectric.prototype={utils:{/**
       * Detect mobile browser
       *
       * @return {boolean}
       */isMobile:function isMobile(){return /android|ip(hone|od|ad)/i.test(navigator.userAgent);},/**
       * Escape especial characters in string (https://developer.mozilla.org/en-US/docs/Web/JavaScript/Guide/Regular_Expressions)
       *
       * @param  {string} str - The string to be escaped
       * @return {string}       The string with the special characters escaped
       */escapeRegExp:function escapeRegExp(str){return str.replace(/[.*+?^${}()|[\]\\]/g,'\\$&');// $& means the whole matched string
},/**
       * Replace diacritics
       *
       * @param  {string} str - The string to replace the diacritics
       * @return {string}       The string with diacritics replaced with ascii characters
       */replaceDiacritics:function replaceDiacritics(str){var k=diacritics.length;while(k--){str=str.toLowerCase().replace(diacritics[k],chars[k]);}return str;},/**
       * Format string
       * https://gist.github.com/atesgoral/984375
       *
       * @param  {string} f - String to be formated
       * @return {string}     String formated
       */format:function format(f){var a=arguments;// store outer arguments
...
:a[i];// assume argument index and return i-th argument
       ...
       */nextEnabledItem:function nextEnabledItem(selectItems,selected){while(selectItems[selected=(selected+1)%selectItems.length].disabled){// empty
}return selected;},/**
       ...
       */previousEnabledItem:function previousEnabledItem(selectItems,selected){while(selectItems[selected=(selected>0?selected:selectItems.length)-1].disabled){// empty
}return selected;},/**
       * Transform camelCase string to dash-case.
       *
       * @param  {string} str - The camelCased string.
       * @return {string}       The string transformed to dash-case.
       */toDash:function toDash(str){return str.replace(/([a-z0-9])([A-Z])/g,'$1-$2').toLowerCase();},/**
       * Calls the events registered with function name.
       *
       * @param {string}    fn - The name of the function.
       * @param {number} scope - Scope that should be set on the function.
       */triggerCallback:function triggerCallback(fn,scope){var elm=scope.element;var func=scope.options['on'+fn];var args=[elm].concat([].slice.call(arguments).slice(1));if($.isFunction(func)){func.apply(elm,args);}$(elm).trigger(pluginName+'-'+this.toDash(fn),args);},/**
       * Transform array list to concatenated string and remove empty values
       * @param  {array} arr - Class list
       * @return {string}      Concatenated string
       */arrayToClassname:function arrayToClassname(arr){var newArr=$.grep(arr,function(item){return!!item;});return $.trim(newArr.join(' '));}},/** Initializes */init:function init(opts){var _this=this;// Set options
_this.options=$.extend(true,{},$.fn[pluginName].defaults,_this.options,opts);_this.utils.triggerCallback('BeforeInit',_this);// Preserve data
_this.destroy(true);// Disable on mobile browsers
if(_this.options.disableOnMobile&&_this.utils.isMobile()){_this.disableOnMobile=true;return;}// Get classes
_this.classes=_this.getClassNames();// Create elements
var input=$('<input/>',{'class':_this.classes.input,'readonly':_this.utils.isMobile()});var items=$('<div/>',{'class':_this.classes.items,'tabindex':-1});var itemsScroll=$('<div/>',{'class':_this.classes.scroll});var wrapper=$('<div/>',{'class':_this.classes.prefix,'html':_this.options.arrowButtonMarkup});var label=$('<span/>',{'class':'label'});var outerWrapper=_this.$element.wrap('<div/>').parent().append(wrapper.prepend(label),items,input);var hideSelectWrapper=$('<div/>',{'class':_this.classes.hideselect});_this.elements={input:input,items:items,itemsScroll:itemsScroll,wrapper:wrapper,label:label,outerWrapper:outerWrapper};if(_this.options.nativeOnMobile&&_this.utils.isMobile()){_this.elements.input=undefined;hideSelectWrapper.addClass(_this.classes.prefix+'-is-native');_this.$element.on('change',function(){_this.refresh();});}_this.$element.on(_this.eventTriggers).wrap(hideSelectWrapper);_this.originalTabindex=_this.$element.prop('tabindex');_this.$element.prop('tabindex',-1);_this.populate();_this.activate();_this.utils.triggerCallback('Init',_this);},/** Activates the plugin */activate:function activate(){var _this=this;var hiddenChildren=_this.elements.items.closest(':visible').children(':hidden').addClass(_this.classes.tempshow);var originalWidth=_this.$element.width();hiddenChildren.removeClass(_this.classes.tempshow);_this.utils.triggerCallback('BeforeActivate',_this);_this.elements.outerWrapper.prop('class',_this.utils.arrayToClassname([_this.classes.wrapper,_this.$element.prop('class').replace(/\S+/g,_this.classes.prefix+'-$&'),_this.options.responsive?_this.classes.responsive:'']));if(_this.options.inheritOriginalWidth&&originalWidth>0){_this.elements.outerWrapper.width(originalWidth);}_this.unbindEvents();if(!_this.$element.prop('disabled')){_this.state.enabled=true;// Not disabled, so... Removing disabled class
_this.elements.outerWrapper.removeClass(_this.classes.disabled);// Remove styles from items box
// Fix incorrect height when refreshed is triggered with fewer options
_this.$li=_this.elements.items.removeAttr('style').find('li');_this.bindEvents();}else{_this.elements.outerWrapper.addClass(_this.classes.disabled);if(_this.elements.input){_this.elements.input.prop('disabled',true);}}_this.utils.triggerCallback('Activate',_this);},/**
     * Generate classNames for elements
     *
     * @return {object} Classes object
     */getClassNames:function getClassNames(){var _this=this;var customClass=_this.options.customClass;var classesObj={};$.each(classList.split(' '),function(i,currClass){var c=customClass.prefix+currClass;classesObj[currClass.toLowerCase()]=customClass.camelCase?c:_this.utils.toDash(c);});classesObj.prefix=customClass.prefix;return classesObj;},/** Set the label text */setLabel:function setLabel(){var _this=this;var labelBuilder=_this.options.labelBuilder;if(_this.state.multiple){// Make sure currentValues is an array
...
currentValues=currentValues.length===0?[0]:currentValues;var labelMarkup=$.map(currentValues,function(value){return $.grep(_this.lookupItems,function(item){return item.index===value;})[0];// we don't want nested arrays here
});labelMarkup=$.grep(labelMarkup,function(item){// Hide default (please choose) if more then one element were selected.
// If no option value were given value is set to option text by default
if(labelMarkup.length>1||labelMarkup.length===0){return $.trim(item.value)!=='';}return item;});labelMarkup=$.map(labelMarkup,function(item){return $.isFunction(labelBuilder)?labelBuilder(item):_this.utils.format(labelBuilder,item);});// Limit the amount of selected values shown in label
if(_this.options.multiple.maxLabelEntries){if(labelMarkup.length>=_this.options.multiple.maxLabelEntries+1){labelMarkup=labelMarkup.slice(0,_this.options.multiple.maxLabelEntries);labelMarkup.push($.isFunction(labelBuilder)?labelBuilder({text:'...'}):_this.utils.format(labelBuilder,{text:'...'}));}else{labelMarkup.slice(labelMarkup.length-1);}}_this.elements.label.html(labelMarkup.join(_this.options.multiple.separator));}else{var currItem=_this.lookupItems[_this.state.currValue];_this.elements.label.html($.isFunction(labelBuilder)?labelBuilder(currItem):_this.utils.format(labelBuilder,currItem));}},/** Get and save the available options */populate:function populate(){var _this=this;var $options=_this.$element.children();var $justOptions=_this.$element.find('option');var $selected=$justOptions.filter(':selected');var selectedIndex=$justOptions.index($selected);var currIndex=0;var emptyValue=_this.state.multiple?[]:0;if($selected.length>1&&_this.state.multiple){selectedIndex=[];$selected.each(function(){selectedIndex.push($(this).index());});}_this.state.currValue=~selectedIndex?selectedIndex:emptyValue;_this.state.selectedIdx=_this.state.currValue;_this.state.highlightedIdx=_this.state.currValue;_this.items=[];_this.lookupItems=[];if($options.length){// Build options markup
$options.each(function(i){var $elm=$(this);if($elm.is('optgroup')){var optionsGroup={element:$elm,label:$elm.prop('label'),groupDisabled:$elm.prop('disabled'),items:[]};$elm.children().each(function(i){var $elm=$(this);optionsGroup.items[i]=_this.getItemData(currIndex,$elm,optionsGroup.groupDisabled||$elm.prop('disabled'));_this.lookupItems[currIndex]=optionsGroup.items[i];currIndex++;});_this.items[i]=optionsGroup;}else{_this.items[i]=_this.getItemData(currIndex,$elm,$elm.prop('disabled'));_this.lookupItems[currIndex]=_this.items[i];currIndex++;}});_this.setLabel();_this.elements.items.append(_this.elements.itemsScroll.html(_this.getItemsMarkup(_this.items)));}},/**
     * Generate items object data
     * @param  {integer} index      - Current item index
     * @param  {node}    $elm       - Current element node
     * @param  {boolean} isDisabled - Current element disabled state
     * @return {object}               Item object
     */getItemData:function getItemData(index,$elm,isDisabled){var _this=this;return{index:index,element:$elm,value:$elm.val(),className:$elm.prop('class'),text:$elm.html(),slug:$.trim(_this.utils.replaceDiacritics($elm.html())),alt:$elm.attr('data-alt'),selected:$elm.prop('selected'),disabled:isDisabled};},/**
     * Generate options markup
     *
     * @param  {object} items - Object containing all available options
     * @return {string}         HTML for the options box
     */getItemsMarkup:function getItemsMarkup(items){var _this=this;var markup='<ul>';if($.isFunction(_this.options.listBuilder)&&_this.options.listBuilder){items=_this.options.listBuilder(items);}$.each(items,function(i,elm){if(elm.label!==undefined){markup+=_this.utils.format('<ul class="{1}"><li class="{2}">{3}</li>',_this.utils.arrayToClassname([_this.classes.group,elm.groupDisabled?'disabled':'',elm.element.prop('class')]),_this.classes.grouplabel,elm.element.prop('label'));$.each(elm.items,function(i,elm){markup+=_this.getItemMarkup(elm.index,elm);});markup+='</ul>';}else{markup+=_this.getItemMarkup(elm.index,elm);}});return markup+'</ul>';},/**
     * Generate every option markup
     *
     * @param  {number} index    - Index of current item
     * @param  {object} itemData - Current item
     * @return {string}            HTML for the option
     */getItemMarkup:function getItemMarkup(index,itemData){var _this=this;var itemBuilder=_this.options.optionsItemBuilder;// limit access to item data to provide a simple interface
// to most relevant options.
var filteredItemData={value:itemData.value,text:itemData.text,slug:itemData.slug,index:itemData.index};return _this.utils.format('<li data-index="{1}" class="{2}">{3}</li>',index,_this.utils.arrayToClassname([itemData.className,index===_this.items.length-1?'last':'',itemData.disabled?'disabled':'',itemData.selected?'selected':'']),$.isFunction(itemBuilder)?_this.utils.format(itemBuilder(itemData,this.$element,index),itemData):_this.utils.format(itemBuilder,filteredItemData));},/** Remove events on the elements */unbindEvents:function unbindEvents(){var _this=this;_this.elements.wrapper.add(_this.$element).add(_this.elements.outerWrapper).add(_this.elements.input).off(eventNamespaceSuffix);},/** Bind events on the elements */bindEvents:function bindEvents(){var _this=this;_this.elements.outerWrapper.on('mouseenter'+eventNamespaceSuffix+' mouseleave'+eventNamespaceSuffix,function(e){$(this).toggleClass(_this.classes.hover,e.type==='mouseenter');// Delay close effect when openOnHover is true
if(_this.options.openOnHover){clearTimeout(_this.closeTimer);if(e.type==='mouseleave'){_this.closeTimer=setTimeout($.proxy(_this.close,_this),_this.options.hoverIntentTimeout);}else{_this.open();}}});// Toggle open/close
_this.elements.wrapper.on('click'+eventNamespaceSuffix,function(e){_this.state.opened?_this.close():_this.open(e);});// Translate original element focus event to dummy input.
// Disabled on mobile devices because the default option list isn't
// shown due the fact that hidden input gets focused
if(!(_this.options.nativeOnMobile&&_this.utils.isMobile())){_this.$element.on('focus'+eventNamespaceSuffix,function(){_this.elements.input.focus();});_this.elements.input.prop({tabindex:_this.originalTabindex,disabled:false}).on('keydown'+eventNamespaceSuffix,$.proxy(_this.handleKeys,_this)).on('focusin'+eventNamespaceSuffix,function(e){_this.elements.outerWrapper.addClass(_this.classes.focus);// Prevent the flicker when focusing out and back again in the browser window
_this.elements.input.one('blur',function(){_this.elements.input.blur();});if(_this.options.openOnFocus&&!_this.state.opened){_this.open(e);}}).on('focusout'+eventNamespaceSuffix,function(){_this.elements.outerWrapper.removeClass(_this.classes.focus);}).on('input propertychange',function(){var val=_this.elements.input.val();var searchRegExp=new RegExp('^'+_this.utils.escapeRegExp(val),'i');// Clear search
clearTimeout(_this.resetStr);_this.resetStr=setTimeout(function(){_this.elements.input.val('');},_this.options.keySearchTimeout);if(val.length){// Search in select options
$.each(_this.items,function(i,elm){if(elm.disabled){return;}if(searchRegExp.test(elm.text)||searchRegExp.test(elm.slug)){_this.highlight(i);return;}if(!elm.alt){return;}var altItems=elm.alt.split('|');for(var ai=0;ai<altItems.length;ai++){if(!altItems[ai]){break;}if(searchRegExp.test(altItems[ai].trim())){_this.highlight(i);return;}}});}});}_this.$li.on({// Prevent <input> blur on Chrome
mousedown:function mousedown(e){e.preventDefault();e.stopPropagation();},click:function click(){_this.select($(this).data('index'));// Chrome doesn't close options box if select is wrapped with a label
// We need to 'return false' to avoid that
return false;}});},/**
     * Behavior when keyboard keys is pressed
     *
     * @param {object} e - Event object
     */handleKeys:function handleKeys(e){var _this=this;var key=e.which;var keys=_this.options.keys;var isPrevKey=$.inArray(key,keys.previous)>-1;var isNextKey=$.inArray(key,keys.next)>-1;var isSelectKey=$.inArray(key,keys.select)>-1;var isOpenKey=$.inArray(key,keys.open)>-1;var idx=_this.state.highlightedIdx;var isFirstOrLastItem=isPrevKey&&idx===0||isNextKey&&idx+1===_this.items.length;var goToItem=0;// Enter / Space
if(key===13||key===32){e.preventDefault();}// If it's a directional key
if(isPrevKey||isNextKey){if(!_this.options.allowWrap&&isFirstOrLastItem){return;}if(isPrevKey){goToItem=_this.utils.previousEnabledItem(_this.lookupItems,idx);}if(isNextKey){goToItem=_this.utils.nextEnabledItem(_this.lookupItems,idx);}_this.highlight(goToItem);}// Tab / Enter / ESC
if(isSelectKey&&_this.state.opened){_this.select(idx);if(!_this.state.multiple||!_this.options.multiple.keepMenuOpen){_this.close();}return;}// Space / Enter / Left / Up / Right / Down
if(isOpenKey&&!_this.state.opened){_this.open();}},/** Update the items object */refresh:function refresh(){var _this=this;_this.populate();_this.activate();_this.utils.triggerCallback('Refresh',_this);},/** Set options box width/height */setOptionsDimensions:function setOptionsDimensions(){var _this=this;// Calculate options box height
// Set a temporary class on the hidden parent of the element
var hiddenChildren=_this.elements.items.closest(':visible').children(':hidden').addClass(_this.classes.tempshow);var maxHeight=_this.options.maxHeight;var itemsWidth=_this.elements.items.outerWidth();var wrapperWidth=_this.elements.wrapper.outerWidth()-(itemsWidth-_this.elements.items.width());// Set the dimensions, minimum is wrapper width, expand for long items if option is true
if(!_this.options.expandToItemText||wrapperWidth>itemsWidth){_this.finalWidth=wrapperWidth;}else{// Make sure the scrollbar width is included
_this.elements.items.css('overflow','scroll');// Set a really long width for _this.elements.outerWrapper
_this.elements.outerWrapper.width(9e4);_this.finalWidth=_this.elements.items.width();// Set scroll bar to auto
_this.elements.items.css('overflow','');_this.elements.outerWrapper.width('');}_this.elements.items.width(_this.finalWidth).height()>maxHeight&&_this.elements.items.height(maxHeight);// Remove the temporary class
hiddenChildren.removeClass(_this.classes.tempshow);},/** Detect if the options box is inside the window */isInViewport:function isInViewport(){var _this=this;if(_this.options.forceRenderAbove===true){_this.elements.outerWrapper.addClass(_this.classes.above);}else if(_this.options.forceRenderBelow===true){_this.elements.outerWrapper.addClass(_this.classes.below);}else{var scrollTop=$win.scrollTop();var winHeight=$win.height();var uiPosX=_this.elements.outerWrapper.offset().top;var uiHeight=_this.elements.outerWrapper.outerHeight();var fitsDown=uiPosX+uiHeight+_this.itemsHeight<=scrollTop+winHeight;var fitsAbove=uiPosX-_this.itemsHeight>scrollTop;// If it does not fit below, only render it
// above it fit's there.
// It's acceptable that the user needs to
// scroll the viewport to see the cut off UI
var renderAbove=!fitsDown&&fitsAbove;var renderBelow=!renderAbove;_this.elements.outerWrapper.toggleClass(_this.classes.above,renderAbove);_this.elements.outerWrapper.toggleClass(_this.classes.below,renderBelow);}},/**
     * Detect if currently selected option is visible and scroll the options box to show it
     *
     * @param {Number|Array} index - Index of the selected items
     */detectItemVisibility:function detectItemVisibility(index){var _this=this;var $filteredLi=_this.$li.filter('[data-index]');if(_this.state.multiple){// If index is an array, we can assume a multiple select and we
// want to scroll to the uppermost selected item!
// Math.min.apply(Math, index) returns the lowest entry in an Array.
index=$.isArray(index)&&index.length===0?0:index;index=$.isArray(index)?Math.min.apply(Math,index):index;}var liHeight=$filteredLi.eq(index).outerHeight();var liTop=$filteredLi[index].offsetTop;var itemsScrollTop=_this.elements.itemsScroll.scrollTop();var scrollT=liTop+liHeight*2;_this.elements.itemsScroll.scrollTop(scrollT>itemsScrollTop+_this.itemsHeight?scrollT-_this.itemsHeight:liTop-liHeight<itemsScrollTop?liTop-liHeight:itemsScrollTop);},/**
...
$('.'+_this.classes.hideselect,'.'+_this.classes.open).children()[pluginName]('close');_this.state.opened=true;_this.itemsHeight=_this.elements.items.outerHeight();_this.itemsInnerHeight=_this.elements.items.height();// Toggle options box visibility
_this.elements.outerWrapper.addClass(_this.classes.open);// Give dummy input focus
_this.elements.input.val('');if(e&&e.type!=='focusin'){_this.elements.input.focus();}// Delayed binds events on Document to make label clicks work
setTimeout(function(){$doc.on('click'+eventNamespaceSuffix,$.proxy(_this.close,_this)).on('scroll'+eventNamespaceSuffix,$.proxy(_this.isInViewport,_this));},1);_this.isInViewport();// Prevent window scroll when using mouse wheel inside items box
if(_this.options.preventWindowScroll){/* istanbul ignore next */$doc.on('mousewheel'+eventNamespaceSuffix+' DOMMouseScroll'+eventNamespaceSuffix,'.'+_this.classes.scroll,function(e){var orgEvent=e.originalEvent;var scrollTop=$(this).scrollTop();var deltaY=0;if('detail'in orgEvent){deltaY=orgEvent.detail*-1;}if('wheelDelta'in orgEvent){deltaY=orgEvent.wheelDelta;}if('wheelDeltaY'in orgEvent){deltaY=orgEvent.wheelDeltaY;}if('deltaY'in orgEvent){deltaY=orgEvent.deltaY*-1;}if(scrollTop===this.scrollHeight-_this.itemsInnerHeight&&deltaY<0||scrollTop===0&&deltaY>0){e.preventDefault();}});}_this.detectItemVisibility(_this.state.selectedIdx);_this.highlight(_this.state.multiple?-1:_this.state.selectedIdx);_this.utils.triggerCallback('Open',_this);}},/** Close the select options box */close:function close(){var _this=this;_this.utils.triggerCallback('BeforeClose',_this);// Remove custom events on document
$doc.off(eventNamespaceSuffix);// Remove visible class to hide options box
_this.elements.outerWrapper.removeClass(_this.classes.open);_this.state.opened=false;_this.utils.triggerCallback('Close',_this);},/** Select current option and change the label */change:function change(){var _this=this;_this.utils.triggerCallback('BeforeChange',_this);if(_this.state.multiple){// Reset old selected
$.each(_this.lookupItems,function(idx){_this.lookupItems[idx].selected=false;_this.$element.find('option').prop('selected',false);});// Set new selected
$.each(_this.state.selectedIdx,function(idx,value){_this.lookupItems[value].selected=true;_this.$element.find('option').eq(value).prop('selected',true);});_this.state.currValue=_this.state.selectedIdx;_this.setLabel();_this.utils.triggerCallback('Change',_this);}else if(_this.state.currValue!==_this.state.selectedIdx){// Apply changed value to original select
...
_this.setLabel();_this.utils.triggerCallback('Change',_this);}},/**
     ...
     */highlight:function highlight(index){var _this=this;var $filteredLi=_this.$li.filter('[data-index]').removeClass('highlighted');_this.utils.triggerCallback('BeforeHighlight',_this);// Parameter index is required and should not be a disabled item
if(index===undefined||index===-1||_this.lookupItems[index].disabled){return;}$filteredLi.eq(_this.state.highlightedIdx=index).addClass('highlighted');_this.detectItemVisibility(index);_this.utils.triggerCallback('Highlight',_this);},/**
     * Select option
     *
     * @param {number} index - Index of the option that will be selected
     */select:function select(index){var _this=this;var $filteredLi=_this.$li.filter('[data-index]');_this.utils.triggerCallback('BeforeSelect',_this,index);// Parameter index is required and should not be a disabled item
if(index===undefined||index===-1||_this.lookupItems[index].disabled){return;}if(_this.state.multiple){// Make sure selectedIdx is an array
_this.state.selectedIdx=$.isArray(_this.state.selectedIdx)?_this.state.selectedIdx:[_this.state.selectedIdx];var hasSelectedIndex=$.inArray(index,_this.state.selectedIdx);if(hasSelectedIndex!==-1){_this.state.selectedIdx.splice(hasSelectedIndex,1);}else{_this.state.selectedIdx.push(index);}$filteredLi.removeClass('selected').filter(function(index){return $.inArray(index,_this.state.selectedIdx)!==-1;}).addClass('selected');}else{$filteredLi.removeClass('selected').eq(_this.state.selectedIdx=index).addClass('selected');}if(!_this.state.multiple||!_this.options.multiple.keepMenuOpen){_this.close();}_this.change();_this.utils.triggerCallback('Select',_this,index);},/**
     * Unbind and remove
     *
     * @param {boolean} preserveData - Check if the data on the element should be removed too
     */destroy:function destroy(preserveData){var _this=this;if(_this.state&&_this.state.enabled){_this.elements.items.add(_this.elements.wrapper).add(_this.elements.input).remove();if(!preserveData){_this.$element.removeData(pluginName).removeData('value');}_this.$element.prop('tabindex',_this.originalTabindex).off(eventNamespaceSuffix).off(_this.eventTriggers).unwrap().unwrap();_this.state.enabled=false;}}};// A really lightweight plugin wrapper around the constructor,
// preventing against multiple instantiations
$.fn[pluginName]=function(args){return this.each(function(){var data=$.data(this,pluginName);if(data&&!data.disableOnMobile){typeof args==='string'&&data[args]?data[args]():data.init(args);}else{$.data(this,pluginName,new Selectric(this,args));}});};/**
   * Default plugin options
   *
   * @type {object}
   */$.fn[pluginName].defaults={onChange:function onChange(elm){$(elm).change();},maxHeight:300,keySearchTimeout:500,arrowButtonMarkup:'<b class="button">&#x25be;</b>',disableOnMobile:false,nativeOnMobile:true,openOnFocus:true,openOnHover:false,hoverIntentTimeout:500,expandToItemText:false,responsive:false,preventWindowScroll:true,inheritOriginalWidth:false,allowWrap:true,forceRenderAbove:false,forceRenderBelow:false,stopPropagation:true,optionsItemBuilder:'{text}',// function(itemData, element, index)
...
},customClass:{prefix:pluginName,camelCase:false},multiple:{separator:', ',keepMenuOpen:true,maxLabelEntries:false}};});/***/},/***/"./node_modules/webpack/buildin/global.js":/*!***********************************!*\
...
module.exports=g;/***/},/***/"./src/js/animations/section1.js":/*!***************************************!*\
  !*** ./src/js/animations/section1.js ***!
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection1Js(module,exports,__webpack_require__){var throttle=__webpack_require__(/*! lodash.throttle */"./node_modules/lodash.throttle/index.js");/**
 * autoAlfa: 0
 * visibilit: hidden;
 */gsap.to('.galaxy',{scrollTrigger:{trigger:".s1",pin:false,start:"top top-=200px",duration:1,toggleActions:"play none reverse none",// end: "+=400",
scrub:false// fix can be in seconds (smooth delay)
// once: true
},y:-300});gsap.to('.s1__box',{scrollTrigger:{trigger:".s1",pin:false,start:"top top-=200px",duration:.9,// end: "+=400",
scrub:false// fix can be in seconds (smooth delay)
// once: true
},y:-100});gsap.to('.s1__image',{scrollTrigger:{trigger:".s1",pin:false,start:"top top=-200px",// end: "+=200",
duration:.9,scrub:false// fix can be in seconds (smooth delay)
// once: true
},y:40});// gsap.fromTo('.header__box, .s1__box', {
//     opacity: 0,
//     y: -100
// }, {
//     y: 0,
...
//     duration: .9
// })
gsap.fromTo(' .s1__title',{opacity:0,x:-100},{x:0,opacity:1,autoAlpha:1,duration:.9});gsap.fromTo(' .s1__subtitle, .s1__btn',{opacity:0,y:100},{y:0,opacity:1,autoAlpha:1,duration:.9});gsap.fromTo('.header__box',{opacity:0,y:-100},{y:0,opacity:1,autoAlpha:1,duration:.9});gsap.fromTo('.galaxy',{opacity:0,y:100},{y:0,opacity:1,autoAlpha:1,duration:.9});gsap.fromTo('.s1__image',{opacity:0,x:100},{x:0,opacity:1,autoAlpha:1,duration:.9});gsap.fromTo('#stars',{scale:1.4,opacity:0},{scale:1,autoAlpha:1,duration:1});function checkGalaxy(){var ofTop=$('.s5').offset().top;var nowScrollTop=$(window).scrollTop();if(nowScrollTop>=ofTop){$('#stars').hide();}else{$('#stars').show();}}checkGalaxy();$(window).on('scroll',throttle(checkGalaxy,200));/***/},/***/"./src/js/animations/section2.js":/*!***************************************!*\
  !*** ./src/js/animations/section2.js ***!
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection2Js(module,exports){gsap.fromTo('.s2',{opacity:0,y:150},{scrollTrigger:{trigger:'.s2',pin:false,start:"top bottom+=50px",end:"+=1000",scrub:false,// fix can be in seconds (smooth delay)
once:true},y:0,opacity:1,duration:.8});/***/},/***/"./src/js/animations/section3.js":/*!***************************************!*\
  !*** ./src/js/animations/section3.js ***!
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection3Js(module,exports){var image1='.s3 .s3__image-1';var image2='.s3 .s3__image-2';gsap.to(image1,{scrollTrigger:{trigger:'.s3',pin:false,start:"top bottom",end:"+=1000",scrub:1.8// fix can be in seconds (smooth delay)
// once: true
},y:'-=160px'});gsap.to(image2,{scrollTrigger:{trigger:'.s3',pin:false,start:"top top",end:"+=1600",scrub:1.8// fix can be in seconds (smooth delay)
// once: true
},y:'-=100px'});/***/},/***/"./src/js/animations/section4.js":/*!***************************************!*\
  !*** ./src/js/animations/section4.js ***!
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection4Js(module,exports){gsap.to('.s4__clouds',{scrollTrigger:{trigger:".s4__clouds",pin:false,start:"top bottom-=300",end:"+=500",scrub:true// fix can be in seconds (smooth delay)
// once: true
},y:'-20%'});/***/},/***/"./src/js/animations/section5.js":/*!***************************************!*\
  !*** ./src/js/animations/section5.js ***!
  \***************************************//*! no static exports found *//***/function srcJsAnimationsSection5Js(module,exports,__webpack_require__){__webpack_require__(/*! ../timer */"./src/js/timer.js");/***/},/***/"./src/js/animations/section6.js":/*!***************************************!*\
  !*** ./src/js/animations/section6.js ***!
  \***************************************//*! no exports provided *//***/function srcJsAnimationsSection6Js(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var _svgModel__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! ../svgModel */"./src/js/svgModel.js");$('.s6-step__num').each(function(index){var svg=new _svgModel__WEBPACK_IMPORTED_MODULE_0__["default"]($(this).find('svg')[0],(index+1)*200);svg.start();});gsap.to('.s6__clouds',{scrollTrigger:{trigger:".s6__clouds",pin:false,start:"top bottom",end:"+=400",scrub:true// fix can be in seconds (smooth delay)
// once: true
},y:'-20%'});/***/},/***/"./src/js/animations/section8.js":/*!***************************************!*\
  !*** ./src/js/animations/section8.js ***!
  \***************************************//*! no exports provided *//***/function srcJsAnimationsSection8Js(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var selectric_public_jquery_selectric__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! selectric/public/jquery.selectric */"./node_modules/selectric/public/jquery.selectric.js");/* harmony import */var selectric_public_jquery_selectric__WEBPACK_IMPORTED_MODULE_0___default=/*#__PURE__*/__webpack_require__.n(selectric_public_jquery_selectric__WEBPACK_IMPORTED_MODULE_0__);var Toggle=function(){function Toggle(){_classCallCheck(this,Toggle);this.items=$('.s8-map__links li a');this.mapEl=$('.s8-map__main-inner');this.listEl=$('.s8-map__list');this.parent=$('.s8-map');this.init();}_createClass(Toggle,[{key:'openMap',value:function openMap(){// this.mapEl.show()
// this.listEl.hide()
this.parent.removeClass('s8-map__main_list');}},{key:'openList',value:function openList(){// this.mapEl.hide()
// this.listEl.show()
this.parent.addClass('s8-map__main_list');}},{key:'addListeners',value:function addListeners(){var self=this;this.items.on('click',function(e){e.preventDefault();if($(this).parent().index()===0){self.openMap();}else{self.openList();}});}},{key:'init',value:function init(){this.addListeners();}}]);return Toggle;}();new Toggle();$('.s8-map__select').selectric();/***/},/***/"./src/js/cursorFollow.js":/*!********************************!*\
  !*** ./src/js/cursorFollow.js ***!
  \********************************//*! no static exports found *//***/function srcJsCursorFollowJs(module,exports){var CursorFollow=function(){function CursorFollow(){_classCallCheck(this,CursorFollow);this.el=document.querySelector('#cursor-follow');this.width=this.el.offsetWidth;this.height=this.el.offsetHeight;this.targetElements=['button','a','.hover','input','select'];this.init();}_createClass(CursorFollow,[{key:'makeActive',value:function makeActive(){this.el.classList.add('active');console.log('active');}},{key:'removeActive',value:function removeActive(){this.el.classList.remove('active');}},{key:'addListeners',value:function addListeners(){var _this4=this;document.body.addEventListener('mousemove',function(e){var x=e.clientX;var y=e.clientY;_this4.el.style.left=x-_this4.width/2+'px';_this4.el.style.top=y-_this4.height/2+'px';});this.targetElements.forEach(function(selector){var elements=[].concat(_toConsumableArray(document.querySelectorAll(selector)));elements.forEach(function(el){el.addEventListener('mouseover',function(){_this4.makeActive();});el.addEventListener('mouseout',function(){_this4.removeActive();});});});}},{key:'init',value:function init(){this.addListeners();}}]);return CursorFollow;}();new CursorFollow();/***/},/***/"./src/js/ismac.js":/*!*************************!*\
  !*** ./src/js/ismac.js ***!
  \*************************//*! no static exports found *//***/function srcJsIsmacJs(module,exports,__webpack_require__){/* WEBPACK VAR INJECTION */(function(global){var ua=navigator.userAgent.toLowerCase();var isMac=ua.indexOf('mac')>-1&&ua.indexOf('os')>-1&&!(ua.indexOf('iphone')>-1||ua.indexOf('ipad')>-1||ua.indexOf('windows')>-1);global.isMac=isMac;/* WEBPACK VAR INJECTION */}).call(this,__webpack_require__(/*! ./../../node_modules/webpack/buildin/global.js */"./node_modules/webpack/buildin/global.js"));/***/},/***/"./src/js/menu.js":/*!************************!*\
  !*** ./src/js/menu.js ***!
  \************************//*! no static exports found *//***/function srcJsMenuJs(module,exports){var Burger=function(){function Burger(_ref3){var open=_ref3.open,close=_ref3.close;_classCallCheck(this,Burger);this.el=$('.header .header__menu-btn');this.menuIcon=this.el.find('.menu-icon');this.ring=this.el.find('.ring');this.isOpen=false;this.onOpen=open||function(){};this.onClose=close||function(){};this.init();}_createClass(Burger,[{key:'makeActive',value:function makeActive(){this.isOpen=true;this.menuIcon.addClass('active');this.runPulse();this.onOpen();}},{key:'runPulse',value:function runPulse(){this.ring.addClass("run");}},{key:'removeActive',value:function removeActive(){this.isOpen=false;this.menuIcon.removeClass('active');this.runPulse();this.ring.removeClass("run");this.onClose();}},{key:'addListeners',value:function addListeners(){var _this5=this;this.el.on('click',function(){if(_this5.isOpen){_this5.removeActive();}else{_this5.makeActive();}});}},{key:'init',value:function init(){this.addListeners();}}]);return Burger;}();var Menu=function(){function Menu(){_classCallCheck(this,Menu);this.burger=new Burger({open:this.open.bind(this),close:this.close.bind(this)});this.wrapperAll=$('.wrapper-all');// this.el = ''
this.init();}_createClass(Menu,[{key:'open',value:function open(){// this.burger.makeActive()
...
gsap.fromTo('.header__menu',{x:'100%',opacity:0},{x:'0',opacity:1,duration:.5,display:'block',ease:'ease-in-out'});}},{key:'close',value:function close(){// this.burger.removeActive()
...
// })
gsap.fromTo('.header__menu',{x:'0',opacity:1},{x:'100%',opacity:0,duration:.5,display:'none'});}},{key:'addListeners',value:function addListeners(){var self=this;$(document).on('click','.scroll-link',function(e){e.preventDefault();var id=$(this).attr('href');var top=$(id).offset().top;var offset=$(this).attr('data-offset')||0;self.burger.removeActive();$('html, body').animate({scrollTop:top+parseInt(offset)},500);});}},{key:'init',value:function init(){this.addListeners();}}]);return Menu;}();new Menu();/***/},/***/"./src/js/parallax.js":/*!****************************!*\
  !*** ./src/js/parallax.js ***!
  \****************************//*! no exports provided *//***/function srcJsParallaxJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! lodash.throttle */"./node_modules/lodash.throttle/index.js");/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default=/*#__PURE__*/__webpack_require__.n(lodash_throttle__WEBPACK_IMPORTED_MODULE_0__);function Parallax(options){var _this6=this;options=options||{};this.nameSpaces={wrapper:options.wrapper||'.parallax',layers:options.layers||'.parallax-layer',deep:options.deep||'data-parallax-deep'};this.onMouseMove=function(e,i){var self=_this6,parallaxWrappers=document.querySelectorAll(_this6.nameSpaces.wrapper);var layers=parallaxWrappers[i].querySelectorAll(self.nameSpaces.layers);for(var j=0;j<layers.length;j++){(function(j){var $this=$(layers[j]);var relX=e.pageX-$this.offset().left;var relY=e.pageY-$this.offset().top;var deep=layers[j].getAttribute(self.nameSpaces.deep),disallow=layers[j].getAttribute('data-parallax-disallow'),x=disallow&&disallow==='x'?0:(relX-$this.width()/2)/$this.width()*deep,y=disallow&&disallow==='y'?0:(relY-$this.height()/2)/$this.height()*deep;if(disallow&&disallow==='both')return;// layers[j].style.transform = 'translateX(' + x + 'px) translateY(' + y + 'px)';
gsap.to(layers[j],{x:x,y:y,duration:1});})(j);}};this.onMouseMove=lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default()(this.onMouseMove.bind(this),200);this.init=function(){var self=this;var parallaxWrappers=document.querySelectorAll(this.nameSpaces.wrapper);for(var i=0;i<parallaxWrappers.length;i++){(function(i){parallaxWrappers[i].addEventListener('mousemove',function(e){self.onMouseMove(e,i);});})(i);}};this.init();return this;}window.addEventListener('load',function(){new Parallax();});/***/},/***/"./src/js/preloader.js":/*!*****************************!*\
  !*** ./src/js/preloader.js ***!
  \*****************************//*! exports provided: default *//***/function srcJsPreloaderJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony export (binding) */__webpack_require__.d(__webpack_exports__,"default",function(){return Loader;});var Loader=function(){function Loader(){_classCallCheck(this,Loader);this.el=$('.preloader');this.items=this.el.children();}_createClass(Loader,[{key:'show',value:function show(){this.el.fadeIn();this.timer=setInterval(function(){// this.items.
},300);}},{key:'hide',value:function hide(){this.el.fadeOut();}}]);return Loader;}();/***/},/***/"./src/js/scripts.js":/*!***************************!*\
  !*** ./src/js/scripts.js ***!
  \***************************//*! no exports provided *//***/function srcJsScriptsJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* WEBPACK VAR INJECTION */(function(global){/* harmony import */var gsap__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! gsap */"./node_modules/gsap/index.js");/* harmony import */var gsap_dist_ScrollTrigger_min_js__WEBPACK_IMPORTED_MODULE_1__=__webpack_require__(/*! gsap/dist/ScrollTrigger.min.js */"./node_modules/gsap/dist/ScrollTrigger.min.js");/* harmony import */var gsap_dist_ScrollTrigger_min_js__WEBPACK_IMPORTED_MODULE_1___default=/*#__PURE__*/__webpack_require__.n(gsap_dist_ScrollTrigger_min_js__WEBPACK_IMPORTED_MODULE_1__);/* harmony import */var _preloader__WEBPACK_IMPORTED_MODULE_2__=__webpack_require__(/*! ./preloader */"./src/js/preloader.js");/* harmony import */var jquery__WEBPACK_IMPORTED_MODULE_3__=__webpack_require__(/*! jquery */"./node_modules/jquery/dist/jquery.js");/* harmony import */var jquery__WEBPACK_IMPORTED_MODULE_3___default=/*#__PURE__*/__webpack_require__.n(jquery__WEBPACK_IMPORTED_MODULE_3__);/* harmony import */var _stars__WEBPACK_IMPORTED_MODULE_4__=__webpack_require__(/*! ./stars */"./src/js/stars.js");__webpack_require__(/*! ./ismac */"./src/js/ismac.js");global.$=jquery__WEBPACK_IMPORTED_MODULE_3___default.a;gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"].registerPlugin(gsap_dist_ScrollTrigger_min_js__WEBPACK_IMPORTED_MODULE_1___default.a);gsap_dist_ScrollTrigger_min_js__WEBPACK_IMPORTED_MODULE_1___default.a.defaults({markers:false});global.gsap=gsap__WEBPACK_IMPORTED_MODULE_0__["gsap"];global.Stars=_stars__WEBPACK_IMPORTED_MODULE_4__["default"];var preloader=new _preloader__WEBPACK_IMPORTED_MODULE_2__["default"]();// preloader.show()
jquery__WEBPACK_IMPORTED_MODULE_3___default()(window).on('load',function(){preloader.hide();__webpack_require__(/*! ./animations/section1 */"./src/js/animations/section1.js");__webpack_require__(/*! ./animations/section2 */"./src/js/animations/section2.js");__webpack_require__(/*! ./animations/section3 */"./src/js/animations/section3.js");__webpack_require__(/*! ./animations/section4 */"./src/js/animations/section4.js");__webpack_require__(/*! ./animations/section5 */"./src/js/animations/section5.js");__webpack_require__(/*! ./animations/section6 */"./src/js/animations/section6.js");__webpack_require__(/*! ./animations/section8 */"./src/js/animations/section8.js");__webpack_require__(/*! ./cursorFollow */"./src/js/cursorFollow.js");__webpack_require__(/*! ./slider1 */"./src/js/slider1.js");__webpack_require__(/*! ./slider2 */"./src/js/slider2.js");__webpack_require__(/*! ./parallax */"./src/js/parallax.js");__webpack_require__(/*! ./menu */"./src/js/menu.js");__webpack_require__(/*! ./scrollTo */"./src/js/scrollTo.js");__webpack_require__(/*! ./sticks */"./src/js/sticks.js");__webpack_require__(/*! ./scrollArrow */"./src/js/scrollArrow.js");// require('./smooth-scroll.js')
__webpack_require__(/*! ./smoothScroll */"./src/js/smoothScroll.js");});var prevScroll=jquery__WEBPACK_IMPORTED_MODULE_3___default()(window).scrollTop();jquery__WEBPACK_IMPORTED_MODULE_3___default()(window).on('scroll',function(e){var nowScrollTop=jquery__WEBPACK_IMPORTED_MODULE_3___default()(window).scrollTop();if(nowScrollTop>prevScroll){_stars__WEBPACK_IMPORTED_MODULE_4__["default"].updateStarsY(-5);}else{_stars__WEBPACK_IMPORTED_MODULE_4__["default"].updateStarsY(5);}prevScroll=nowScrollTop;});/* WEBPACK VAR INJECTION */}).call(this,__webpack_require__(/*! ./../../node_modules/webpack/buildin/global.js */"./node_modules/webpack/buildin/global.js"));/***/},/***/"./src/js/scrollArrow.js":/*!*******************************!*\
  !*** ./src/js/scrollArrow.js ***!
  \*******************************//*! no exports provided *//***/function srcJsScrollArrowJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! lodash.throttle */"./node_modules/lodash.throttle/index.js");/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default=/*#__PURE__*/__webpack_require__.n(lodash_throttle__WEBPACK_IMPORTED_MODULE_0__);var check=function check(){var scrollTop=$(window).scrollTop();if(scrollTop>=60){$('.bottom-btn').addClass('top');}else{$('.bottom-btn').removeClass('top');}};check();$('.bottom-btn').on('click',function(){if($('.bottom-btn').hasClass('top')){$('html, body').animate({scrollTop:0},900);}else{var sTop=$('.s2').offset().top;$('html, body').animate({scrollTop:sTop},900);}});$(window).on('scroll',lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default()(check,300));/***/},/***/"./src/js/scrollTo.js":/*!****************************!*\
  !*** ./src/js/scrollTo.js ***!
  \****************************//*! no static exports found *//***/function srcJsScrollToJs(module,exports){/***/},/***/"./src/js/slider1.js":/*!***************************!*\
  !*** ./src/js/slider1.js ***!
  \***************************//*! no static exports found *//***/function srcJsSlider1Js(module,exports){var Slider=function(){function Slider(){_classCallCheck(this,Slider);this.nowSlide=0;this.prevSlide=0;this.itemsWrapper=$('.s2__items');this.imagesWrapper=$('.s2__images');this.items=this.itemsWrapper.children();this.images=this.imagesWrapper.children();this.tabsWrapper=$('.s2__organs');this.tabs=this.tabsWrapper.children();this.arrowNext=$('.s2 .slider-arrow-next');this.arrowPrev=$('.s2 .slider-arrow-prev');this.isAnimating=false;this.init();}_createClass(Slider,[{key:'setSlide',value:function setSlide(idx){if(this.isAnimating)return;if(idx===this.nowSlide)return;this.prevSlide=this.nowSlide;this.nowSlide=idx;this.animate();}},{key:'next',value:function next(){this.prevSlide=this.nowSlide;if(this.nowSlide>=this.items.length-1){this.nowSlide=0;}else{this.nowSlide++;}}},{key:'prev',value:function prev(){this.prevSlide=this.nowSlide;if(this.nowSlide<=0){this.nowSlide=this.items.length-1;}else{this.nowSlide--;}}},{key:'slideNext',value:function slideNext(){if(this.isAnimating)return;this.next();this.animate();}},{key:'slidePrev',value:function slidePrev(){if(this.isAnimating)return;this.prev();this.animate();}},{key:'animate',value:function animate(){var _this7=this;this.isAnimating=true;if(this.prevSlide<this.nowSlide){Stars.setSpeed(20);}else{Stars.setSpeed(-20);}var resolvers=[];resolvers.push(this.animateText());resolvers.push(this.animateImages());this.animateTabs();Promise.all(resolvers).then(function(){Stars.setSpeed(1);_this7.isAnimating=false;});}},{key:'animateText',value:function animateText(){var _this8=this;// const tl = gsap.timeline({
//     onComplete: () => {
//         Stars.setSpeed(1)
//     }
// })
...
//     display: 'block'
// })
return new Promise(function(resolve){_this8.items.eq(_this8.prevSlide).fadeOut(400,function(){_this8.items.eq(_this8.nowSlide).fadeIn(400,function(){return resolve();});});});}},{key:'animateImages',value:function animateImages(){var _this9=this;var tl=gsap.timeline();// tl.fromTo(this.images.eq(this.prevSlide)[0], {
//     opacity: 1,
// }, {
//     opacity: 0,
//     display: 'none'
...
//     position: 'relative'
// })
return new Promise(function(resolve){_this9.images.eq(_this9.prevSlide).fadeOut(400,function(){_this9.images.eq(_this9.nowSlide).fadeIn(400,function(){return resolve();});});});}},{key:'animateTabs',value:function animateTabs(){this.tabs.removeClass('s2-organ_active');var activeTab=this.tabs.eq(this.nowSlide);var prevTab=this.tabs.eq(this.prevSlide);activeTab.addClass('s2-organ_active');var activeIndexPrev=parseInt(activeTab.attr('data-index'));activeTab.attr('data-index',1);for(var i=activeIndexPrev+1;i<=4;i++){var el=this.tabs.parent().find('.s2-organ[data-index=\''+i+'\']');el.attr('data-index',i-1);}prevTab.attr('data-index',4);}},{key:'addListeners',value:function addListeners(){var _this10=this;var self=this;this.tabs.on('click',function(){var idx=$(this).index();console.log(idx);self.setSlide(idx);});this.arrowNext.on('click',function(){_this10.slideNext();});this.arrowPrev.on('click',function(){_this10.slidePrev();});}},{key:'init',value:function init(){this.addListeners();}}]);return Slider;}();new Slider();/***/},/***/"./src/js/slider2.js":/*!***************************!*\
  !*** ./src/js/slider2.js ***!
  \***************************//*! no static exports found *//***/function srcJsSlider2Js(module,exports){var Slider=function(){function Slider(){_classCallCheck(this,Slider);this.nowSlide=0;this.prevSlide=0;this.itemsWrapper=$('.s4__items');this.imagesWrapper=$('.s4__images');this.items=this.itemsWrapper.children();this.images=this.imagesWrapper.children();this.arrowNext=$('.s4 .slider-arrow-next');this.arrowPrev=$('.s4 .slider-arrow-prev');this.isAnimating=false;this.init();}_createClass(Slider,[{key:'setSlide',value:function setSlide(idx){if(this.isAnimating)return;this.prevSlide=this.nowSlide;this.nowSlide=idx;this.animate();}},{key:'next',value:function next(){this.prevSlide=this.nowSlide;if(this.nowSlide>=this.items.length-1){this.nowSlide=0;}else{this.nowSlide++;}}},{key:'prev',value:function prev(){this.prevSlide=this.nowSlide;if(this.nowSlide<=0){this.nowSlide=this.items.length-1;}else{this.nowSlide--;}}},{key:'slideNext',value:function slideNext(){if(this.isAnimating)return;this.next();this.animate();}},{key:'slidePrev',value:function slidePrev(){if(this.isAnimating)return;this.prev();this.animate();}},{key:'animate',value:function animate(){var _this11=this;this.isAnimating=true;if(this.prevSlide<this.nowSlide){Stars.setSpeed(20);}else{Stars.setSpeed(-20);}var resolvers=[];resolvers.push(this.animateText());resolvers.push(this.animateImages());Promise.all(resolvers).then(function(){Stars.setSpeed(1);_this11.isAnimating=false;});}},{key:'animateText',value:function animateText(){var _this12=this;return new Promise(function(resolve){_this12.items.eq(_this12.prevSlide).fadeOut(400,function(){_this12.items.eq(_this12.nowSlide).fadeIn(400,function(){return resolve();});});});}},{key:'animateImages',value:function animateImages(){var _this13=this;var tl=gsap.timeline();return new Promise(function(resolve){_this13.images.eq(_this13.prevSlide).fadeOut(400,function(){_this13.images.eq(_this13.nowSlide).fadeIn(400,function(){return resolve();});});});}},{key:'addListeners',value:function addListeners(){var _this14=this;var self=this;this.arrowNext.on('click',function(){_this14.slideNext();});this.arrowPrev.on('click',function(){_this14.slidePrev();});}},{key:'init',value:function init(){this.addListeners();}}]);return Slider;}();new Slider();/***/},/***/"./src/js/smooth-scroll.js":/*!*********************************!*\
  !*** ./src/js/smooth-scroll.js ***!
  \*********************************//*! exports provided: default *//***/function srcJsSmoothScrollJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony export (binding) */__webpack_require__.d(__webpack_exports__,"default",function(){return SmoothScroll;});var MathUtils={map:function map(e,t,s,i,r){return(e-t)*(r-i)/(s-t)+i;},lerp:function lerp(e,t,s){return(1-s)*e+s*t;}},body=document.body;var winsize=void 0;var calcWinsize=function calcWinsize(){return winsize={width:window.innerWidth,height:window.innerHeight};};var docScroll=void 0;calcWinsize(),window.addEventListener("resize",calcWinsize);var getPageYScroll=function getPageYScroll(){return docScroll=window.pageYOffset||document.documentElement.scrollTop;};window.addEventListener("scroll",getPageYScroll);var Item=function(){function Item(e){var _this15=this;_classCallCheck(this,Item);this.DOM={el:e},this.DOM.image=this.DOM.el.querySelector(".item__img"),this.renderedStyles={innerTranslationY:{previous:0,current:0,ease:.1,maxValue:parseInt(getComputedStyle(this.DOM.image).getPropertyValue("--overflow"),10),setValue:function setValue(){var e=_this15.renderedStyles.innerTranslationY.maxValue,t=-1*e;return Math.max(Math.min(MathUtils.map(_this15.props.top-docScroll,winsize.height,-1*_this15.props.height,t,e),e),t);}}},this.update(),this.observer=new IntersectionObserver(function(e){e.forEach(function(e){return _this15.isVisible=e.intersectionRatio>0;});}),this.observer.observe(this.DOM.el),this.initEvents();}_createClass(Item,[{key:'update',value:function update(){this.getSize();for(var e in this.renderedStyles){this.renderedStyles[e].current=this.renderedStyles[e].previous=this.renderedStyles[e].setValue();}this.layout();}},{key:'getSize',value:function getSize(){var e=this.DOM.el.getBoundingClientRect();this.props={height:e.height,top:docScroll+e.top};}},{key:'initEvents',value:function initEvents(){var _this16=this;window.addEventListener("resize",function(){return _this16.resize();});}},{key:'resize',value:function resize(){this.update();}},{key:'render',value:function render(){for(var e in this.renderedStyles){this.renderedStyles[e].current=this.renderedStyles[e].setValue(),this.renderedStyles[e].previous=MathUtils.lerp(this.renderedStyles[e].previous,this.renderedStyles[e].current,this.renderedStyles[e].ease);}this.layout();}},{key:'layout',value:function layout(){this.DOM.image.style.transform='translate3d(0,'+this.renderedStyles.innerTranslationY.previous+'px,0)';}}]);return Item;}();var SmoothScroll=function(){function SmoothScroll(){var _this17=this;_classCallCheck(this,SmoothScroll);this.DOM={main:document.querySelector(".data-scroll-box")},this.DOM.scrollable=this.DOM.main.querySelector("div[data-scroll]"),this.items=[],[].concat(_toConsumableArray(this.DOM.main.querySelectorAll(".content > .item"))).forEach(function(e){return _this17.items.push(new Item(e));}),this.renderedStyles={translationY:{previous:0,current:0,ease:.1,setValue:function setValue(){return docScroll;}}},this.setSize(),this.update(),this.style(),this.initEvents(),requestAnimationFrame(function(){return _this17.render();});}_createClass(SmoothScroll,[{key:'update',value:function update(){for(var e in this.renderedStyles){this.renderedStyles[e].current=this.renderedStyles[e].previous=this.renderedStyles[e].setValue();}this.layout();}},{key:'layout',value:function layout(){this.DOM.scrollable.style.transform='translate3d(0,'+-1*this.renderedStyles.translationY.previous+'px,0)';}},{key:'setSize',value:function setSize(){body.style.height=this.DOM.scrollable.scrollHeight+'px';}},{key:'style',value:function style(){this.DOM.main.style.position="fixed",this.DOM.main.style.zIndex=1,this.DOM.main.style.width=this.DOM.main.style.height="100%",this.DOM.main.style.top=this.DOM.main.style.left=0,this.DOM.main.style.overflow="hidden";}},{key:'initEvents',value:function initEvents(){var _this18=this;window.addEventListener("resize",function(){return _this18.setSize();});}},{key:'render',value:function render(){var _this19=this;for(var e in this.renderedStyles){this.renderedStyles[e].current=this.renderedStyles[e].setValue(),this.renderedStyles[e].previous=MathUtils.lerp(this.renderedStyles[e].previous,this.renderedStyles[e].current,this.renderedStyles[e].ease);}this.layout();var _iteratorNormalCompletion=true;var _didIteratorError=false;var _iteratorError=undefined;try{for(var _iterator=this.items[Symbol.iterator](),_step;!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=true){var _e2=_step.value;_e2.isVisible&&_e2.render();}}catch(err){_didIteratorError=true;_iteratorError=err;}finally{try{if(!_iteratorNormalCompletion&&_iterator.return){_iterator.return();}}finally{if(_didIteratorError){throw _iteratorError;}}}requestAnimationFrame(function(){return _this19.render();});}}]);return SmoothScroll;}();var preloadImages=function preloadImages(){return new Promise(function(e,t){});};preloadImages().then(function(){document.body.classList.remove("loading"),getPageYScroll(),new SmoothScroll();}),getPageYScroll();/***/},/***/"./src/js/smoothScroll.js":/*!********************************!*\
  !*** ./src/js/smoothScroll.js ***!
  \********************************//*! no exports provided *//***/function srcJsSmoothScrollJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var _smooth_scroll__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! ./smooth-scroll */"./src/js/smooth-scroll.js");// if (!isMac) {
new _smooth_scroll__WEBPACK_IMPORTED_MODULE_0__["default"]();// }
/***/},/***/"./src/js/stars.js":/*!*************************!*\
  !*** ./src/js/stars.js ***!
  \*************************//*! exports provided: default *//***/function srcJsStarsJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);function random(min,max){return Math.random()*(max-min)+min;}var Star=function(){function Star(){var _ref4=arguments.length>0&&arguments[0]!==undefined?arguments[0]:{},color=_ref4.color,xPos=_ref4.xPos,yPos=_ref4.yPos;_classCallCheck(this,Star);this.defaults={color:'rgba(255, 255, 255, 1)',minRadius:0.5,maxRadius:1.4,minSpeed:.1,maxSpeed:.5,speed:1};this.minRadius=this.defaults.minRadius;this.maxRadius=this.defaults.maxRadius;this.minSpeed=this.defaults.minSpeed;this.maxSpeed=this.defaults.maxSpeed;this.speed=this.defaults.speed;this.radius=this.radius||random(this.minRadius,this.maxRadius);this.xPos=xPos;this.yPos=yPos;this.xVelocity=random(this.minSpeed,this.maxSpeed);this.yVelocity=random(this.minSpeed,this.maxSpeed);this.color=color||this.defaults.color;}_createClass(Star,[{key:'init',value:function init(){}},{key:'getVelocity',value:function getVelocity(){return{x:this.xVelocity*this.speed,y:this.yVelocity*this.speed};}},{key:'setSpeed',value:function setSpeed(speed){this.speed=speed;}}]);return Star;}();var Stars=function(){function Stars(){_classCallCheck(this,Stars);this.numStars=400;this.stars=[];this.canvas=document.getElementById('stars');this.ctx=this.canvas.getContext('2d');// this.ctx.filter = 'blur(300px) '
// this.canvas.style.filter = 'blur(10px)'
this.direction='horizontal';this.requestAnimationFrame=window.requestAnimationFrame;}_createClass(Stars,[{key:'setSpeed',value:function setSpeed(speed){this.stars.forEach(function(star){return star.setSpeed(speed);});}},{key:'setDirection',value:function setDirection(direction){this.direction=direction;}},{key:'speedUp',value:function speedUp(){}},{key:'updateStarsY',value:function updateStarsY(y){var _this20=this;this.stars.forEach(function(star){star.yPos+=y;if(star.yPos<0){star.yPos+=_this20.canvas.height+star.radius;star.xPos=random(0,_this20.canvas.width);}else if(star.yPos>_this20.canvas.height){star.yPos-=_this20.canvas.height+star.radius;star.xPos=random(0,_this20.canvas.width);}});}//Initalize
},{key:'init',value:function init(){this.setCanvasSize();this.createCircle();this.render();}},{key:'setCanvasSize',value:function setCanvasSize(){this.canvas.width=window.innerWidth;this.canvas.height=window.innerHeight;}//Create shape
},{key:'createCircle',value:function createCircle(){var star=[];for(var i=0;i<this.numStars;i++){var self=this;star[i]=new Star({xPos:random(0,this.canvas.width),yPos:random(0,this.canvas.height),color:'rgba(0, '+random(130,160)+', '+random(200,255)+','+random(.7,1)+')'});//once values are determined, draw star on canvas
// self.draw(star, i);
}this.stars=star;}// Draw the stars on canvas
},{key:'draw',value:function draw(i){var self=this,ctx=self.ctx;ctx.fillStyle=this.stars[i].color;// ctx.font = '28pt Calibri';
// ctx.fillStyle = 'white';
...
// ctx.shadowOffsetX = 0;
// ctx.shadowOffsetY = 0;
ctx.arc(this.stars[i].xPos,this.stars[i].yPos,this.stars[i].radius,0,2*Math.PI,false);// ctx.stroke();
ctx.fill();}},{key:'render',value:function render(){var _this21=this;requestAnimationFrame(function(){//clears canvas
_this21.clearCanvas();//redraws stars
for(var i=0;i<_this21.numStars;i++){if(_this21.direction==='horizontal'){_this21.stars[i].xPos-=_this21.stars[i].getVelocity().x;}else{_this21.stars[i].yPos-=_this21.stars[i].getVelocity().y;}//if star goes off screen from the left call reset method
if(_this21.stars[i].xPos<0||_this21.stars[i].xPos>_this21.canvas.width){_this21.resetStar(i);}else if(_this21.stars[i].yPos<0||_this21.stars[i].yPos>_this21.canvas.height){_this21.resetStar(i);}else{_this21.draw(i);}}_this21.render();});}// reset the star xPos with a randon YPos 
},{key:'resetStar',value:function resetStar(i){var self=this;if(this.direction==='horizontal'){if(this.stars[i].xPos<0){this.stars[i].xPos+=this.canvas.width+this.stars[i].radius;}else{this.stars[i].xPos-=this.canvas.width+this.stars[i].radius;}this.stars[i].yPos=random(0,this.canvas.height);}else{if(this.stars[i].yPos<0){this.stars[i].yPos+=this.canvas.height+this.stars[i].radius;}else{this.stars[i].yPos-=this.canvas.height+this.stars[i].radius;}this.stars[i].xPos=random(0,this.canvas.width);}self.draw(i);}},{key:'clearCanvas',value:function clearCanvas(){this.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);}}]);return Stars;}();var stars=new Stars();stars.init();/* harmony default export */__webpack_exports__["default"]=stars;/***/},/***/"./src/js/sticks.js":/*!**************************!*\
  !*** ./src/js/sticks.js ***!
  \**************************//*! no static exports found *//***/function srcJsSticksJs(module,exports,__webpack_require__){var throttle=__webpack_require__(/*! lodash.throttle */"./node_modules/lodash.throttle/index.js");var menuItems=$('.header__link');var switcherItems=$('.switcher__item');switcherItems.on('click',function(){var idx=$(this).index();var activeItem=menuItems.eq(idx);var activeSection=activeItem.attr('href');var top=$(activeSection).offset().top;var offset=activeItem.attr('data-offset')||0;$('html, body').animate({scrollTop:top+parseInt(offset)},300);});var check=function check(){switcherItems.removeClass('switcher__item_active');menuItems.removeClass('header__link_active');switcherItems.eq(0).addClass('switcher__item_active');menuItems.each(function(index){var id=$(this).attr('href');var offset=$(this).attr('data-offset')||0;var sTop=$(id).offset().top;var scrollTop=$(window).scrollTop();if(scrollTop>=sTop+parseInt(offset)-10){switcherItems.eq(index).addClass('switcher__item_active');menuItems.eq(index).addClass('header__link_active');}else{}});};$(window).on('scroll',throttle(check,300));/***/},/***/"./src/js/svgModel.js":/*!****************************!*\
  !*** ./src/js/svgModel.js ***!
  \****************************//*! exports provided: default *//***/function srcJsSvgModelJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony export (binding) */__webpack_require__.d(__webpack_exports__,"default",function(){return SvgModel;});/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! lodash.throttle */"./node_modules/lodash.throttle/index.js");/* harmony import */var lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default=/*#__PURE__*/__webpack_require__.n(lodash_throttle__WEBPACK_IMPORTED_MODULE_0__);var SvgModel=function(){function SvgModel(el,speed){_classCallCheck(this,SvgModel);this.el=$(el);this.items=this.el.find('circle');this.nowActive=0;this.speed=speed||300;}_createClass(SvgModel,[{key:'start',value:function start(){var _this22=this;if(this.timer)this.clearInterval(this.timer);var tick=function tick(){_this22.next();_this22.makeActive();// this.timer = requestAnimationFrame(tick, this.speed)
};tick=lodash_throttle__WEBPACK_IMPORTED_MODULE_0___default()(tick,this.speed);this.timer=setInterval(tick,this.speed);// tick()
}},{key:'stop',value:function stop(){clearTimeout(this.timer);}},{key:'reset',value:function reset(){this.nowActive=0;this.items.attr('fill-opacity','0.18');this.items.css('filter','');}},{key:'next',value:function next(){this.nowActive++;if(this.nowActive>=this.items.length){this.reset();}}},{key:'makeActive',value:function makeActive(){var activeItem=this.items.eq(this.nowActive);activeItem.attr('fill-opacity','1');activeItem.css('filter','url(#dropshadow)');}}]);return SvgModel;}();/***/},/***/"./src/js/timer.js":/*!*************************!*\
  !*** ./src/js/timer.js ***!
  \*************************//*! no exports provided *//***/function srcJsTimerJs(module,__webpack_exports__,__webpack_require__){"use strict";__webpack_require__.r(__webpack_exports__);/* harmony import */var _svgModel__WEBPACK_IMPORTED_MODULE_0__=__webpack_require__(/*! ./svgModel */"./src/js/svgModel.js");var date=new Date(2020,11,30,12);var Timer=function(){function Timer(){_classCallCheck(this,Timer);this.date=date;this.daysWrapper=$('.s5-timer__value_1');this.daysNum=this.daysWrapper.find('.s5-timer__value-num');this.daysText=this.daysWrapper.find('.s5-timer__value-text');this.daysSvg=this.daysWrapper.find('svg');this.hoursWrapper=$('.s5-timer__value_2');this.hoursNum=this.hoursWrapper.find('.s5-timer__value-num');this.hoursText=this.hoursWrapper.find('.s5-timer__value-text');this.hoursSvg=this.hoursWrapper.find('svg');this.minutesWrapper=$('.s5-timer__value_3');this.minutesNum=this.minutesWrapper.find('.s5-timer__value-num');this.minutesText=this.minutesWrapper.find('.s5-timer__value-text');this.minutesSvg=this.minutesWrapper.find('svg');this.daysTimer=new _svgModel__WEBPACK_IMPORTED_MODULE_0__["default"](this.daysSvg,500);this.hoursTimer=new _svgModel__WEBPACK_IMPORTED_MODULE_0__["default"](this.hoursSvg,400);this.minutesTimer=new _svgModel__WEBPACK_IMPORTED_MODULE_0__["default"](this.minutesSvg,300);this.init();}_createClass(Timer,[{key:'getTimeRemaining',value:function getTimeRemaining(){var t=Date.parse(this.date)-Date.parse(new Date());var seconds=Math.floor(t/1000%60);var minutes=Math.floor(t/1000/60%60);var hours=Math.floor(t/(1000*60*60)%24);var days=Math.floor(t/(1000*60*60*24));return{'total':t,'days':days,'hours':hours,'minutes':minutes,'seconds':seconds};}},{key:'wordForm',value:function wordForm(num,word){var cases=[2,0,1,1,1,2];return word[num%100>4&&num%100<20?2:cases[num%10<5?num%10:5]];}},{key:'setDays',value:function setDays(days){this.daysNum.text(days);var daysText=this.wordForm(days,['день','дня','дней']);this.daysText.text(daysText);}},{key:'setMinutes',value:function setMinutes(min){this.minutesNum.text(min);var minText=this.wordForm(min,['минута','минуты','минут']);this.minutesText.text(minText);}},{key:'setHours',value:function setHours(h){this.hoursNum.text(h);var secText=this.wordForm(h,['час','часа','часов']);this.hoursText.text(secText);}},{key:'start',value:function start(){var _this23=this;var tick=function tick(){var _getTimeRemaining=_this23.getTimeRemaining(),days=_getTimeRemaining.days,hours=_getTimeRemaining.hours,minutes=_getTimeRemaining.minutes,seconds=_getTimeRemaining.seconds;_this23.setDays(days);_this23.setHours(hours);_this23.setMinutes(minutes);};this.timer=setInterval(tick,1000);tick();}},{key:'init',value:function init(){this.daysTimer.start();this.minutesTimer.start();this.hoursTimer.start();this.start();}}]);return Timer;}();new Timer();/***/}/******/});
